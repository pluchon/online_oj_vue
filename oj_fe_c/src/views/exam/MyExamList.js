...
export default defineComponent({
  ...
  setup() {
    ...
    const getStatusInfo = (item) => {
      ...
      // 1. 已完赛
      if (item.contestStatus === 2 || (end > 0 && now > end)) {
        return {
          phase: 'ended',
          text: '已完赛',
          badgeClass: 'ended',
      ...
      // 2. 进行中
      if (item.contestStatus === 1 || (start > 0 && now >= start && (end === 0 || now <= end))) {
        return {
          phase: 'ongoing',
          text: '进行中',
          badgeClass: 'ongoing',
    ...
    const ongoingCount = computed(() => {
      return myExamList.value.filter(item => getStatusInfo(item).phase === 'ongoing').length
    })
    const upcomingCount = computed(() => {
      return myExamList.value.filter(item => getStatusInfo(item).phase === 'upcoming').length
    })
    const endedCount = computed(() => {
      return myExamList.value.filter(item => getStatusInfo(item).phase === 'ended').length
    })

    // 加载我的竞赛列表
    const loadMyExamList = async (page) => {
    ...
    onMounted(() => {
      if (isLogin.value) {
        ...
        router.push({ path: '/login', query: { redirect: '/my-exam' } })
      }
      nextTick(() => {
        updateSliderIndicator()
      })
